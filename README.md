...
The current pull-request intake remains the reviewed `figure-library.publication-submission.v1` fixed-R contract. A separate trusted-`main` workflow verifies two future mixed-runtime renderer images for the 0.7 protocol from a 222-artifact exact URL/SHA-256 lock and digest-pinned OCI foundations. The images have not been built on this Windows development machine and do not enter Archive intake until trusted-Linux verification, real GHCR digests, validator support, and a later reviewed policy change are all available. Pull-request workflows never receive `packages: write`.
